...
将 OpenWrt-NanoPiNEO2-OLED-SPI-0.1.3-Develop-ext4.img.gz 解压后所得文件是 ext4fs 类型的分区映像，可挂载后复制文件到指定目标分区（以下假定目标分区为 /dev/sda3）。

        gzip -d OpenWrt-NanoPiNEO2-OLED-SPI-0.1.3-Develop-ext4.img.gz
        mkdir -p /tmp/src
        mkdir -p /tmp/dst
